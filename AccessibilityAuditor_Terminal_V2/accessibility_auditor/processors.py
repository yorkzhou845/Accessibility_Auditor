...
def process_semantic_structure_document(
    doc,
    pdf_path,
    failure_report,
    reference_context="",
    ...
    if not cfg.USE_LLM_FOR_SEMANTIC:
        ...
        ai = {"remediation_script": script, "confidence": 0.65}
        warnings.append(
            "Used deterministic heading detection. LLM semantic refinement is disabled."
        )
    else:
        ...
        if ai.get("_parse_error") or "remediation_script" not in ai:
            ai = {
                "remediation_script": make_fallback_remediation_script(heading_candidates),
                "confidence": 0.55,
            }
            warnings.append(
                "Used the deterministic fallback because Ollama did not return valid JSON."
            )
        else:
            ...
            if not script and heading_candidates:
                script = make_fallback_remediation_script(heading_candidates)
                warnings.append(
                    "No valid model entries matched the candidates; used the deterministic fallback."
                )
        ...
        "remediations": [
            {
                ...
                "issue": "Document may need a corrected heading hierarchy.",
                "recommended_action": (
                    "Review the suggested heading script and apply approved tags with "
                    "a PDF remediation library or editor."
                ),
                "proposed_change": {
...
def process_alt_text_document(
    ...
    for page_index in range(page_count):
        ...
        print(f"Processing page {page_number} for alternative text...", flush=True)
            ...
            page_result["remediations"].append(
                {
                    ...
                    "recommended_action": (
                        "Review the suggestion and add approved alternative text with "
                        "a PDF remediation library or editor."
                    ),
                    "proposed_change": {
...
def process_table_summary_document(
    doc,
    pdf_path,
    failure_report,
    reference_context="",
            ...
            page_result["remediations"].append(
                {
                    ...
                    "issue": "Table may need a clearer representation or concise description.",
                    "recommended_action": (
                        "Review the Markdown and summary, then apply approved changes with "
                        "a PDF remediation library or editor."
                    ),
                    "proposed_change": {
...
def unsupported_result(pdf_path, classification_reason):
    return {
        "document_name": pdf_path.name,
        "task": "unsupported",
        ...
        "remediations": [],
        "warnings": [
            "No remediation suggestion was generated because the reported issue is outside the supported tasks."
        ],
    }
